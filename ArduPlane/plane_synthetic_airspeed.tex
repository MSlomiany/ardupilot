...
\end{equation}
where \texttt{rotMat} is exactly the matrix returned by \texttt{\_ahrs.get\_rotation\_body\_to\_ned()}, \texttt{rotMat.c.x} is exactly the matrix element used in the implementation, and $a_x$ is the measured body-axis forward acceleration from \texttt{AP::ins().get\_accel().x}.
In the library's convention, \texttt{rotMat.c.x} is the $(x,c)$ entry of that body-to-NED direction cosine matrix, i.e. the NED-$x$ component of the matrix's third column. The note keeps this implementation-level convention explicitly so the sign and axis mapping match the code exactly.
